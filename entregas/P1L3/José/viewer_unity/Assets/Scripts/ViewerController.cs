...
namespace Mcoc.UnityViewer
{
    ...
    public partial class ViewerController : MonoBehaviour
    {
        ...
        List<double> DiagramForceVector(AnalysisElementResult result, bool firstEnd)
        {
            ...
            // El diagrama usa una cara interna común: conserva i e invierte j.
            // Los valores crudos continúan disponibles en el inspector.
        ...
        public void RunP1L4DemoSequenceCheck()
        {
            ...
            if (beam == null) failures.Add("viga con crosswalk histórico no encontrada");
            else
            {
                ShowInfo(beam);
                ...
                if (!diagramCaption.Contains("END_FORCES_INTERPOLATION") || !diagramCaption.Contains("sin cargas interiores"))
                    failures.Add("clasificación física del diagrama no visible");
                ...
                if (auditedRows.Count == 0) failures.Add(auditedBeamId + " no tiene miembro FE histórico");
                else
                {
                    ...
                    if (rawJ == null || diagramJ == null || rawJ.Count != diagramJ.Count)
                        failures.Add("vectores i/j de " + auditedBeamId + " no disponibles");
                    else
                        for (int index = 0; index < rawJ.Count; index++)
                            if (System.Math.Abs(rawJ[index] + diagramJ[index]) > 1e-9)
                                failures.Add("conversión de j a cara interna común inválida");
                }
            }
            ...
            if (failures.Count == 0)
                Debug.Log("[P1L4 DEMO QA] PASS: VIGA identidad/ejes/R/fuerzas/cara-interna/deformada/My-2D/N-V-2D; COLUMNA P-M/demanda; MURO P-M/ASUMIDO_LAB; GLOBAL cargas/apoyos/tributarias.");
