...
def main():
    ...
    report=["# FASE 7 — candidato topológico POST-P1L3","","Estado: `CANDIDATE_NOT_APPROVED_NOT_RUN`. No reemplaza A3-A4.","","## Estrategia","","Los muros conservan un miembro vertical equivalente en su centro para no multiplicar rigidez. Los encuentros comprobados generan nodos sobre el eje y restricciones rígidas internas al muro. Las vigas se segmentan solo en incidencias de huellas o cruces del mismo edificio y nivel.","","## Métricas","",f"- Flotantes: 72 geometrías/50 componentes → {q['candidate_floating_geometry_elements']}/{q['candidate_floating_components']}.",f"- Nodos: 731 en la reevaluación histórica → {q['node_count']} en el candidato.",f"- Elementos FE: 1041 incluidos históricos → {q['fe_element_count']} candidatos.",f"- Restricciones internas: {q['constraint_count']}.",f"- Geometrías con relación 1:N: {q['geometry_elements_split_into_multiple_fe']}; máximo {q['max_fe_segments_per_geometry']} segmentos.",f"- Viga–muro: {q['beam_wall_connections']}; muro–muro: {q['wall_wall_connections']}; solapes verticales: {q['wall_vertical_overlaps']}; columna–viga: {q['column_beam_connections']}.","","## Clasificación de los 72 elementos foco","",*[f"- `{k}`: {v}." for k,v in q["classification_counts"].items()],"",f"- `REAL_CANTILEVER`: {ids('REAL_CANTILEVER','structural_classification')}.",f"- `TRANSFERRED`: {ids('TRANSFERRED','structural_classification')}.",f"- `FE_ADAPTER_ERROR`: {ids('FE_ADAPTER_ERROR','structural_classification')}.",f"- `UNRESOLVED`: {ids('UNRESOLVED','structural_classification')}.","","## Validación","",*[f"- `{k}`: {v}." for k,v in q["validation_counts"].items()],"",f"Crosswalk: `{q['crosswalk_required_fields']}` con `element_id`, `geometry_elementTag`, `analysis_id`, `opensees_element_tag` y nodos OpenSees. No se ejecutó OpenSees ni se modificaron cargas, resultados, geometría o Unity."]
