...
namespace Mcoc.Semana2.UnityE1
{
    public class E1ViewerController : MonoBehaviour
    {
        ...
        private void DrawLeftControlPanel()
        {
            ...
            GUILayout.BeginArea(new Rect(10, 10, GuiWidth, height), "E1 Global / Controls", GUI.skin.window);
            ...
            if (responseLoaded)
            {
                ...
                if (showDeformedBlocked) GUILayout.Label("WARNING: blocked/scoping deformation is not physically verified.");
        ...
        private void DrawGlobalQaPanel()
        {
            ...
            GUI.color = original;
            GUILayout.Label("Global Equilibrium (QA)");
            ...
            GUILayout.Label(string.Format(CultureInfo.InvariantCulture, "Relative error: {0:F4}%", qa.relative_error_pct.GetValueOrDefault()));
            ...
            if (responseData.max_displacement != null)
            {
        ...
        private void LoadStructuralMappingData()
        {
            ...
            if (mappingData == null || mappingData.mappings == null) return;
            if (mappingData.mappings.elements != null)
            {
                foreach (KeyValuePair<string, E1StructuralElementMapping> pair in mappingData.mappings.elements)
                {
                    if (pair.Value != null) structuralElementMappingByVisualId[pair.Key] = pair.Value;
                }
            }
            if (mappingData.mappings.supports != null)
            {
                foreach (KeyValuePair<string, E1StructuralSupportMapping> pair in mappingData.mappings.supports)
                {
                    if (pair.Value != null) structuralSupportMappingByVisualId[pair.Key] = pair.Value;
                }
            }
            if (mappingData.mappings.nodes != null)
            {
                foreach (KeyValuePair<string, E1StructuralNodeMapping> pair in mappingData.mappings.nodes)
                {
        ...
        private void BuildResponseVisualizations()
        {
            ...
            foreach (KeyValuePair<string, E1ResponseElement> pair in responseData.elements)
            {
                ...
                bool blocked = E1AnalysisStatus.IsBlocking(element.analysis_status);
                ...
                else if (!verified && showDeformedBlocked && (scoping || blocked))
                {
        ...
        {
            foreach (GameObject go in deformedLines) if (go != null) go.SetActive(showDeformed);
            foreach (GameObject go in deformedBlockedLines) if (go != null) go.SetActive(showDeformedBlocked);
            foreach (GameObject go in diagramLines) if (go != null) go.SetActive(showDiagrams);
            bool loadsVisible = showLoads && selectedView != null && FloorMatches(selectedView.floorId);
        ...
        private void AppendSupportProperties(StringBuilder sb, E1ElementView view)
        {
            ...
            E1SupportRestraintResponse restraint = SupportRestraint(feNode);
            E1ReactionResponse reaction = SupportReaction(feNode);
        ...
        private E1SupportRestraintResponse SupportRestraint(string feNode)
        {
            if (responseData == null || responseData.support_restraints == null || string.IsNullOrWhiteSpace(feNode)) return null;
            return responseData.support_restraints.TryGetValue(feNode, out E1SupportRestraintResponse restraint) ? restraint : null;
        }

        private E1ReactionResponse SupportReaction(string feNode)
        {
            if (responseData == null || responseData.reactions_kN == null || string.IsNullOrWhiteSpace(feNode)) return null;
            return responseData.reactions_kN.TryGetValue(feNode, out E1ReactionResponse reaction) ? reaction : null;
        }
