...
| SLAB-E1-001 — S1/P1/P2/P3 | Bounding boxes provisionales | DXF directo RLE-LOSA, vigas, ejes, notas y huecos | P2 candidato 806.605 m² y P3 914.175 m², ambos sin lados exteriores pendientes; S1 sin borde exterior RLE y P1 con transición outboard no resuelta | Apariencia, áreas y PP | P0 | Muros/vigas por piso | Auditar S1/P1 con plantas/elevaciones complementarias y clasificar huecos antes de aplicar | IN_PROGRESS |
| SLAB-E2-001 — S1-P4 | Bounding boxes provisionales | Serie 2024_22 exclusivamente | Propuesta exterior: S1–P3 565.392 m² y P4 566.465 m², 0 lados sin respaldo; rasgos interiores siguen `REVIEW_REQUIRED_HOLE_OR_PANEL_EDGE` | Apariencia, áreas y PP | P0 | Geometría ED2 estable | Resolver huecos/paños interiores antes de aplicar; `losa-ne` no implica ausencia de losa | IN_PROGRESS |
...
| Issue | Origen | Evidencia disponible | Estado actual | Impacto | Prioridad | Dependencia | Solución propuesta | Estado |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PROP-BEAM-001 | Etiquetas CAD y anchos por contorno | Propuesta ED1 y `cad_property_audit.json` | ED1: ancho 300/300; altura 281 con evidencia directa/familia inequívoca y 19 `UNKNOWN` | Rigidez y fuerzas FE | P1 | GEO-BEAM-E1-001 | Resolver solo las 19 alturas restantes desde detalle/elevación; no usar profundidad visual como sección | IN_PROGRESS |
...
### FASE 3 — vigas EDIFICIO_1

- La lectura directa de `553` segmentos `RLE-VIGA` confirmó que el extractor
  histórico representaba mayoritariamente las dos caras del contorno.
- La propuesta validada produce `300` centrolineas analíticas: S1 `53`, P1
  `69`, P2 `64`, P3 `60` y P4 `54`. Se excluyeron `46` cierres cortos y `4`
  detalles interiores; no quedan caras largas ni diagonales sin resolver.
- Los `545` prismas históricos fueron reemplazados conservando un ID por
  centrolinea y registrando el crosswalk. El ancho es trazable en 300/300;
  `281` alturas tienen evidencia directa o de familia inequívoca y `19` permanecen
  `UNKNOWN`. Para estas últimas, 0.60 m es solo profundidad visual declarada.
- EDIFICIO_1 queda con `522` sólidos y el combinado con `1210`. Propuesta,
  aplicación, geometría combinada, continuidad, muros, diff de Luis y auditoría
